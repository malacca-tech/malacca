...
public class FreeMarker {
    ...
    public String parseExpression(String config) {
        ...
        try {
            ...
            throw new FreeMarkerMessageHandlerException("FreeMarker输入输出异常:" + e.getMessage()
                    + config
                    , e);
        } catch (TemplateException e) {
            throw new FreeMarkerMessageHandlerException("FreeMarker模板异常:" + e.getMessage()
                    + config
                    , e);
        } catch (Exception e) {
            throw new FreeMarkerMessageHandlerException("FreeMarker语法处理异常:" + e.getMessage()
                    + config
                    , e);
        } finally {
